...
# Interested in looking at metabolic genes (esp. in AT2 cells)

genes:

glycolysis:
Aldoa, Aldob, Aldoc, Bpgm, Eno1, Eno2, Eno3, Gapdhs, Gpi1, Hk2, Hk3, Pfkl, Pgk1, Pgk2,
Pgm1, Pgm2, Pgm3, Pklr, Tpi1.

TCA:
Aco1, Aco2, Citrate synthase, Dlat, Dld, Dlst, Fh1, Idh1, Idh2, Idh3a, Idh3b,
Idh3g, Mdh1, Mdh1b, Mdh2, Ogdh, Pck1, Pck2, Pcx, Pdha1, Pdhb, Sdha, Sdhb, Sdhc, Sdhd, Sucla2,
Suclg1, Suclg2.

'hemoxygenase' genes are also of interest?

'hmox1' hmox2 is there too but shouldnt be changing too much.
...
hmox <- all_genes_bm %>%
  filter(str_detect(description, "heme oxygenase"))
```
...
# This chunk will take the Seurat object as an input and filter is such that each seurat cluster is in its own seurat object and then make dotplots of the top markers across the experimental conditions...
```{r}
...
lapply(seq_along(seurat.list.top.markers), function(i) write.csv(seurat.list.top.markers[[i]], file = paste0("/Users/jwalla12/Repositories/dennery_scrna/scRNAseq/tables/cluster_", i-1, "_markers.csv"), row.names = F))
...
```{r AT1 cells}
all_data.combined_25 <- subset(all_data.combined, subset = seurat_clusters =="25")
DefaultAssay(all_data.combined_25) <- 'SCT'

all_data.combined_25.markers <- FindAllMarkers(all_data.combined_25, min.pct = 0.25, logfc.threshold = 0.25, assay = 'SCT')
all_data.combined_25.top_markers <- all_data.combined_25.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_logFC)
DotPlot(all_data.combined_25, features = c(unique(all_data.combined_25.top_markers$gene)))  + RotatedAxis()



all_data.combined_25.markers <- FindMarkers(all_data.combined_25, min.pct = 0.25, logfc.threshold = 0.25, assay = 'SCT', ident.1 ='AirD60', ident.2 = '')
```
```{r ciliated}
all_data.combined_28 <- subset(all_data.combined, subset = seurat_clusters =="28")
DefaultAssay(all_data.combined_28) <- 'SCT'

all_data.combined_28.markers <- FindAllMarkers(all_data.combined_28, min.pct = 0.25, logfc.threshold = 0.25, assay = 'SCT')
all_data.combined_28.top_markers <- all_data.combined_28.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_logFC)
DotPlot(all_data.combined_28, features = c(unique(all_data.combined_28.top_markers$gene)))  + RotatedAxis()
```


```{r club/clara}
all_data.combined_39 <- subset(all_data.combined, subset = seurat_clusters =="39")
DefaultAssay(all_data.combined_39) <- 'SCT'

all_data.combined_39.markers <- FindAllMarkers(all_data.combined_39, min.pct = 0.25, logfc.threshold = 0.25, assay = 'SCT')
all_data.combined_39.top_markers <- all_data.combined_39.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_logFC)
DotPlot(all_data.combined_39, features = c(unique(all_data.combined_39.top_markers$gene)))  + RotatedAxis()
```
...
```{r AT2 - clust 3}
all_data.combined_3 <- subset(all_data.combined, subset = seurat_clusters =="3")
DefaultAssay(all_data.combined_3) <- 'SCT'

all_data.combined_3.markers <- FindAllMarkers(all_data.combined_3, min.pct = 0.25, logfc.threshold = 0.25, assay = 'SCT')
all_data.combined_3.top_markers <- all_data.combined_3.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_logFC)
DotPlot(all_data.combined_3, features = c(unique(all_data.combined_3.top_markers$gene)))  + RotatedAxis()
```


```{r AT2 - clust 13}
all_data.combined_13 <- subset(all_data.combined, subset = seurat_clusters =="13")
DefaultAssay(all_data.combined_13) <- 'SCT'

all_data.combined_13.markers <- FindAllMarkers(all_data.combined_13, min.pct = 0.25, logfc.threshold = 0.25, assay = 'SCT')
all_data.combined_13.top_markers <- all_data.combined_13.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_logFC)
DotPlot(all_data.combined_13, features = c(unique(all_data.combined_13.top_markers$gene)))  + RotatedAxis()
```

```{r AT2 - clust 11}
all_data.combined_11 <- subset(all_data.combined, subset = seurat_clusters =="11")
DefaultAssay(all_data.combined_11) <- 'SCT'

all_data.combined_11.markers <- FindAllMarkers(all_data.combined_11, min.pct = 0.25, logfc.threshold = 0.25, assay = 'SCT')
all_data.combined_11.top_markers <- all_data.combined_11.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_logFC)
DotPlot(all_data.combined_11, features = c(unique(all_data.combined_11.top_markers$gene)))  + RotatedAxis()
```
