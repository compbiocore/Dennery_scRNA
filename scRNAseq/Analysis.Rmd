...
all_data.combined$mca_encoded<-as.factor(all_data.combined$atlas_predicted_clusters_short)
...
levels(all_data.combined$mca_encoded)[levels(all_data.combined$mca_encoded)=="Club Cell"] <- "9"
...
all_data.combined$mca_encoded_translated <- paste0(all_data.combined$mca_encoded, "_", all_data.combined$atlas_predicted_clusters_short)

all_data.combined$mca_encoded <- paste0("mca_", all_data.combined$mca_encoded)
...
#gsave(plot = p6, file = 'mca_encoded.pdf', width = 10, height = 10, units = 'in')
p6
...
#grep('Spc', rownames(all_data.combined), value = TRUE)
library('plyr')

features <- c('Spock2', 'Alas2', 'Tkt', 'Malat1', 'Cftr','Spc24', 'Spc25')
...
AT2_seurat <- subset(all_data.combined, seurat_clusters == '3' | seurat_clusters == '11' | seurat_clusters == '13'| seurat_clusters == '22'| seurat_clusters == '27')                        
at2_tsne <- DimPlot(AT2_seurat, reduction = 'tsne', group.by = 'seurat_clusters', split.by = 'orig.ident', ncol = 4) & theme(text = element_text(size=20), axis.text.y = element_text(size=20))

at2_tsne
#ggsave('tsne_3-11-13-22-17.pdf', plot = at2_tsne, height = 4, width = 10)

AT2_O2D7_seurat <- subset(AT2_seurat, orig.ident == 'O2D7')
AT2_AirD7_seurat <- subset(AT2_seurat, orig.ident == 'AirD7')
AT2_O2D60_seurat <- subset(AT2_seurat, orig.ident == 'O2D60')
AT2_AirD60_seurat <- subset(AT2_seurat, orig.ident == 'AirD60')

#this first one is to just get a scale bar for spock
spock_legend <- FeaturePlot(AT2_AirD7_seurat, features = features[1], reduction = 'tsne')& theme(text = element_text(size=20), axis.text.y = element_text(size=20)) & scale_colour_gradientn(limits= c(0,2.5), colors = c('bisque', 'darkred'))

spock_plot <- FeaturePlot(AT2_seurat, ncol = 4, split.by = 'orig.ident', features = features[1], reduction = 'tsne') & scale_colour_gradientn(limits= c(0,2.5), colors = c('bisque', 'darkred'))

#ggsave('spock_plot.pdf', plot = spock_plot, height = 4, width = 10)
#ggsave('spock_legend.pdf', plot = spock_legend, height = 4, width = 10)

#this first one is to just get a scale bar for alas2
alas2_legend <- FeaturePlot(AT2_O2D60_seurat, features = features[2], reduction = 'tsne')& theme(text = element_text(size=20), axis.text.y = element_text(size=20)) & scale_colour_gradientn(limits= c(0,2), colors = c('bisque', 'darkred'))

alas2_plot <- FeaturePlot(AT2_seurat, ncol = 4, split.by = 'orig.ident', features = features[2], reduction = 'tsne') & scale_colour_gradientn(limits= c(0,2), colors = c('bisque', 'darkred'))

#ggsave('alas2_plot.pdf', plot = alas2_plot, height = 4, width = 10)
#ggsave('alas2_legend.pdf', plot = alas2_legend, height = 4, width = 10)

#this first one is to just get a scale bar for tkt
tkt_legend <- FeaturePlot(AT2_AirD60_seurat, features = features[3], reduction = 'tsne') & theme(text = element_text(size=20), axis.text.y = element_text(size=20)) & scale_colour_gradientn(limits= c(0,2), colors = c('bisque', 'darkred'))

tkt_plot <- FeaturePlot(AT2_seurat, ncol = 4, split.by = 'orig.ident', features = features[3], reduction = 'tsne') & scale_colour_gradientn(limits= c(0,2), colors = c('bisque', 'darkred'))

ggsave('tkt_plot.pdf', plot = tkt_plot, height = 4, width = 10)
ggsave('tkt_legend.pdf', plot = tkt_legend, height = 4, width = 10)


#this first one is to just get a scale bar for Malat1
malat1_legend <- FeaturePlot(AT2_O2D60_seurat, features = features[4], reduction = 'tsne') & theme(text = element_text(size=20), axis.text.y = element_text(size=20)) & scale_colour_gradientn(limits= c(0,6), colors = c('bisque', 'darkred'))

malat1_plot <- FeaturePlot(AT2_seurat, ncol = 4, split.by = 'orig.ident', features = features[4], reduction = 'tsne') & scale_colour_gradientn(limits= c(0,6), colors = c('bisque', 'darkred'))

ggsave('malat1_plot.pdf', plot = malat1_plot, height = 4, width = 10)
ggsave('malat1_legend.pdf', plot = malat1_legend, height = 4, width = 10)



#this first one is to just get a scale bar for ctfr
ctfr_legend <- FeaturePlot(AT2_AirD60_seurat, features = features[5], reduction = 'tsne') & theme(text = element_text(size=20), axis.text.y = element_text(size=20)) & scale_colour_gradientn(limits= c(0,1.6), colors = c('bisque', 'darkred'))

ctfr_plot <- FeaturePlot(AT2_seurat, ncol = 4, split.by = 'orig.ident', features = features[5], reduction = 'tsne') & scale_colour_gradientn(limits= c(0,1.6), colors = c('bisque', 'darkred'))

ggsave('ctfr_plot.pdf', plot = ctfr_plot, height = 4, width = 10)
ggsave('ctfr_legend.pdf', plot = ctfr_legend, height = 4, width = 10)


#this first one is to just get a scale bar for spc24
spc24_legend <- FeaturePlot(AT2_O2D60_seurat, features = features[6], reduction = 'tsne') & theme(text = element_text(size=20), axis.text.y = element_text(size=20)) & scale_colour_gradientn(limits= c(0,1), colors = c('bisque', 'darkred'))

spc24_plot <- FeaturePlot(AT2_seurat, ncol = 4, split.by = 'orig.ident', features = features[6], reduction = 'tsne') & scale_colour_gradientn(limits= c(0,1), colors = c('bisque', 'darkred'))

ggsave('spc24_plot.pdf', plot = spc24_plot, height = 4, width = 10)
ggsave('spc24_legend.pdf', plot = spc24_legend, height = 4, width = 10)



#this first one is to just get a scale bar for spc24
spc25_legend <- FeaturePlot(AT2_AirD7_seurat, features = features[7], reduction = 'tsne') & scale_colour_gradientn(limits= c(0,1), colors = c('bisque', 'darkred'))

spc25_plot <- FeaturePlot(AT2_seurat, ncol = 4, split.by = 'orig.ident', features = features[7], reduction = 'tsne') & scale_colour_gradientn(limits= c(0,1), colors = c('bisque', 'darkred'))

ggsave('spc25_plot.pdf', plot = spc25_plot, height = 4, width = 10)
ggsave('spc25_legend.pdf', plot = spc25_legend, height = 4, width = 10)

```

```{r Trying out ridge plots and dimplots }

RidgePlot(AT2_seurat, features = features, ncol = 7, group.by = 'seurat_clusters')
RidgePlot(all_data.combined, features = features, ncol = 7, group.by = 'predicted.ids')

VlnPlot(AT2_seurat, features = features[1])

DotPlot(all_data.combined, features = features, split.by = 'seurat_clusters', cols = c('blue', 'green', 'red', 'orange', 'purple'))


RidgePlot(all_data.combined, features = features[4], ncol = 7, group.by = 'atlas_predicted_clusters')


DimPlot(AT2_seurat, reduction="tsne", group.by = "atlas_predicted_clusters") 
FeaturePlot(AT2_seurat)
#FeaturePlot(O2D60_seurat, ncol = 5, features = features, reduction = 'tsne', cells = c(colnames(AT2_seurat))) & scale_colour_gradientn(limits= c(0,8), colors = c('bisque', 'darkred'))
